  ...
  return createPortal(
    <div className="fixed inset-0 z-[2000] flex items-center justify-center">
       ...
       <motion.div
          ...
          className={`
             relative w-full h-full md:h-[95vh] md:w-[95vw] md:max-w-none md:rounded-3xl
       ...
       >
          ...
          <motion.button
             ...
             whileTap={{ scale: 0.9 }}
             onClick={onClose}
             className="absolute top-4 right-4 z-50 p-2 bg-black/20 hover:bg-black/40 text-white rounded-full backdrop-blur-sm transition-colors cursor-pointer border border-white/20"
          ...
          <div className="flex-1 overflow-y-auto bg-white scrollbar-hide">
             <motion.div
                className="p-6 md:p-10 lg:p-12 max-w-2xl mx-auto space-y-8"
             ...
             >
                 ...
                 <motion.div variants={itemVariants} className="flex flex-wrap gap-3">
                     ...
                     <button
                        ...
                        className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-700 px-6 py-3.5 rounded-xl font-bold transition-all active:scale-95 hover:-translate-y-0.5 border border-slate-200"
                     >
                        <Share2 size={18} />
                        Share
                     </button>
                 ...
                 {/* Description */}
                 <motion.div variants={itemVariants} className="prose prose-lg prose-slate text-slate-600 leading-relaxed">
                     <p>{location.description}</p>
                 </motion.div>

                 {/* Celestial Content */}
                 {isCelestial && (
                     <motion.div variants={itemVariants} className="p-6 bg-purple-50 rounded-2xl border border-purple-100">
                         ...
                         <div className="grid grid-cols-1 gap-3">
                             {nearbyLocations.map(loc => (
                                 <motion.div
                                    ...
                                    key={loc.id}
                                    className="flex items-center gap-4 p-3 rounded-2xl bg-white transition-colors border border-slate-100 group cursor-pointer shadow-sm"
